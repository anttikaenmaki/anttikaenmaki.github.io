...
What is the analogue of [Theorem 2](#theorem-2) for the Assouad spectrum? For the Assouad *dimension* on compact sets, the set of realisable projection functions is essentially the upper semi-continuous functions. For the Assouad spectrum on unbounded sets, [Theorem 6](#theorem-6) only constructs two prescribed values; one expects a full classification analogous to [Theorem 2](#theorem-2), but the brick construction needs to be pushed considerably further to deliver it.
